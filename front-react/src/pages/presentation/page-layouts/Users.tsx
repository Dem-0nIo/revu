...
// Validation schema for Primer Nombre
...
const Users = () => {
	...
	useEffect(() => {
		axios
		  .get('/api/roles')
		  .then((response) => setRoles(response.data as Role[])) // Cast response data to Role[]
		  .catch((error) => console.error('Error fetching roles:', error));
	  }, []);

	const handleSubmit = (values: any) => {
		console.log('Formulario enviado:', values);
		// Call backend service here to save data
	};
											...
											<div className='col-md-6'>
												<FormGroup id='rol' label='Primer Nombre' isFloating>													
													<Field as='select' name='role' className='form-select'>
														<select id="rol" name="rol" value={formik.values.rol} onChange={formik.handleChange} className="form-select">
